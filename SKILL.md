...
Options:
  ...
  --parallel         Independent analysis mode: all providers review simultaneously
                     with no chained critique. Faster, broader coverage, but loses
                     adversarial chain depth. Stage 2 only (ignored in Stage 1).
...
5. `--parallel` → PARALLEL_MODE=true (플래그 존재 시), 기본값 false
6. 나머지 전체 → 검증 대상 텍스트 (TOPIC)

**파싱 결과 구조**:
- `parsed_flags`: {tier, role, stage, id, parallel}
- `topic`: 검증 대상 텍스트
...
## Step 0-H: Parallel 모드 사전 검증 (PARALLEL_MODE=true 시만 실행)

PARALLEL_MODE=false 이면 이 섹션을 건너뛴다.

**Stage 1 충돌 경고**:
```
STAGE == 1이고 PARALLEL_MODE == true 이면:
  ⚠ --parallel ignored: Stage 1 uses a single Claude model with no external
    reviewers to parallelize. Run /gauntlet --setup to configure Stage 2 providers.
  PARALLEL_MODE=false로 초기화하고 계속 진행한다.
```

**실행 모드 확인 출력** (Stage 2 + PARALLEL_MODE=true):
```
🔀 Independent Analysis Mode — --parallel enabled
  Each provider reviews r0-draft independently (no chained critique).
  Note: Claude provider = self-review (same vendor as author).
  Trade-off: faster & broader vs. default chained adversarial depth.
```
...
**PARALLEL_MODE=true (Stage 2)** → Read 툴: `{GAUNTLET_SKILL_DIR}/ref-pipeline-parallel.md`

```
⚠ Read 실패 시:
🚨 ref file missing: ref-pipeline-parallel.md
Path: ~/.claude/skills/gauntlet/ref-pipeline-parallel.md
Next action: reinstall gauntlet or check the path
```
→ **세션 없이 stderr 출력 후 중단** (session.json 접근 없음)

Read 성공 시 → 해당 파일의 Step 1~8 절차를 따라 실행한다.

---

**PARALLEL_MODE=false (기본 직렬)** → Read 툴: `{GAUNTLET_SKILL_DIR}/ref-pipeline.md`
